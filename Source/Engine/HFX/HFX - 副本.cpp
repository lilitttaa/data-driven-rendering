...
void StringBuffer::AppendFormat(const char* format, ...)
{
	...
	while (true)
	{
		...
		{
			data_.insert(data_.end(), temp.begin(), temp.begin() + writtenChars);
			break;
		}
...
DataBuffer::DataBuffer(uint32_t maxEntries, uint32_t bufferSize): entries_(maxEntries), data_(bufferSize, '\0'),
	max_entries_(maxEntries), current_entry_trail_index_(0),
	buffer_size_(bufferSize), current_size_(0) {}
...
void Lexer::GetTokenTextFromString(IndirectString& tokenText)
{
	...
	tokenText.length_= static_cast<uint32_t>(position - tokenText.text);
...
void Lexer::NextToken(Token& token)
{
	...
	else
	{
		if (IsIdOrKeyword(c))
		{
			...
			token.text.length_= static_cast<uint32_t>(position - token.text.text);
		}
...
void Parser::DeclarationGlsl()
{
	...
	codeFragment.code.length_= token.text.text - codeFragment.code.text;
...
bool Parser::NumberAndIdentifier(Token& token)
{
	...
	if (token.type == TokenType::Token_Number)
	{
		...
		token.text.length_+= numberToken.text.length;
	}
...
const CodeFragment* Parser::FindCodeFragment(const IndirectString& name)
{
	for (uint32_t i = 0; i < ast.codeFragments.size(); ++i)
	{
		...
		if (IndirectString::Equals(name, type->name)) { return type; }
	}
	return nullptr;
}

ShaderGenerator::ShaderGenerator(const AST& ast): ast_(ast) {}

void ShaderGenerator::GenerateShaders(const std::string& path)
{
	const uint32_t passCount = (uint32_t)ast_.passes_.size();
	for (uint32_t i = 0; i < passCount; i++)
	{
		const Pass& pass = ast_.passes_[i];
...
void ShaderGenerator::OutputShaderStage(const std::string& path, const Pass::Stage& stage)
{
	...
	std::string fileName = path + codeFragment->name_.ToString() + "_" + ShaderStage2Postfix(stage.stage) + ".glsl";
	std::ofstream file(fileName);
	std::string code = codeFragment->code_.ToString();
